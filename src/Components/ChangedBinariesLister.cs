...
public class ChangedBinariesLister(IBinariesHelper binariesHelper, IMsBuilder msBuilder,
    ...
    private async Task<IList<BinaryToUpdate>> ListChangedBinariesAsync(string repositoryId, string branchId, string previousHeadTipIdSha, string currentHeadTipIdSha,
        ...
        foreach (bool previous in new[] { true, false}) {
            ...
            foreach (string csProjFile in csProjFiles) {
                var contents = (await File.ReadAllLinesAsync(csProjFile)).ToList();
                contents = contents.Select(AdjustLineIfVersioningRelated).Select(MakeDeterministic).ToList();
                await File.WriteAllLinesAsync(csProjFile, contents);
            }

            RemoveNonSourceCodeFiles(compileFolder, folderCleanUpErrorsAndInfos);
        ...
        foreach (string shortFileName in Directory.GetFiles(currentTargetFolder.FullName, "*.*", SearchOption.AllDirectories).Select(f => f.Substring(currentTargetFolder.FullName.Length + 1))) {
            ...
            byte[] previousContents = await File.ReadAllBytesAsync(previousFileName);
            byte[] currentContents = await File.ReadAllBytesAsync(currentFileName);
    ...
    private void RemoveNonSourceCodeFiles(IFolder compileFolder, IErrorsAndInfos errorsAndInfos) {
