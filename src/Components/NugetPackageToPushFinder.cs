...
public class NugetPackageToPushFinder : INugetPackageToPushFinder {
    ...
    public async Task<IPackageToPush> FindPackageToPushAsync(string nugetFeedId, IFolder packageFolderWithBinaries, IFolder repositoryFolder, string solutionFileFullName, string branchId, IErrorsAndInfos errorsAndInfos) {
        ...
        if (!string.IsNullOrEmpty(remotePackage.Tags) && !string.IsNullOrWhiteSpace(headTipIdSha)) {
            ...
            var changedBinaries = _ChangedBinariesLister.ListChangedBinaries(packageId, branchId, headTipIdSha, tag, listerErrorsAndInfos);
